...
snowsurvey_csv %>%
  count(Snow_cover) %>%
  filter(is.na(as.numeric(Snow_cover))) # filter out non-numeric when column is chr
...
typeof(snowsurvey_fixed$Snow_cover)
```
...
```{r}
 snowsurvey_fixed %>% filter(Snow_cover > 100 | Snow_cover < 0)
```
